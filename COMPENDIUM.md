...
What apps get **today** for testing their own code: nothing yet. Writing a handler test means manually constructing a Cabinet container, registering services, faking PSR-7 requests. This is the next big arc — see PLAN.md's "Testing utilities + PSR-20 Clock adoption" entry, which will land `TestKernel`, `Factory`, and `Fake\` namespaces under `Arcanum\Test\`. The Clock half of that arc is in progress — Vault, Throttle, and Auth (CliSession) all consume `Hourglass\Clock` now, so any test that touches them can pass a `FrozenClock` and assert TTL/expiry behavior deterministically without `sleep()`.
