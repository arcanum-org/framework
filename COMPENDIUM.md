...
- **Shodo** — Output formatting. `JsonFormatter`, `HtmlFormatter`, `CsvFormatter`, `PlainTextFormatter`, `MarkdownFormatter`, `KeyValueFormatter`, `TableFormatter`, plus their fallback variants. `TemplateCompiler` translates `{{ }}` syntax into PHP via regex passes; the body of `{{ }}` is treated as a real PHP expression with helper-call rewriting. Directives: `{{ extends 'layout' }}` / `{{ section }}` / `{{ yield }}` for layout inheritance, `{{ include }}` for partials, `{{ fragment 'name' }} … {{ endfragment }}` for named addressable regions within a section (used by the htmx package for partial rendering — `HtmlFormatter::renderFragment()` renders just one fragment by name, with independent per-fragment caching and soft fall-through on unknown names). `HelperRegistry`, `HelperResolver` (domain-scoped via co-located `Helpers.php`), `HelperDiscovery`, plus the `#[WithHelper]` per-DTO attribute.
