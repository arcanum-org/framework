...
final class TemplateCompilerTest extends TestCase
{
    ...
    // ------------------------------------------------------------------
    // Named fragments: {{ fragment 'name' }} … {{ endfragment }}
    // ------------------------------------------------------------------

    public function testNamedFragmentExtractsContentByName(): void
    {
        // Arrange
        $compiler = new TemplateCompiler();
        $source = file_get_contents(self::$fixtureDir . '/page-with-fragments.html');
        assert(is_string($source));

        // Act
        $result = $compiler->compile($source, self::$fixtureDir, fragmentName: 'sidebar');

        // Assert — only the sidebar fragment, no layout, no product list
        $this->assertStringNotContainsString('<!DOCTYPE html>', $result);
        $this->assertStringNotContainsString('<h1>Products</h1>', $result);
        $this->assertStringNotContainsString('foreach', $result);
        $this->assertStringContainsString('$__escape((string)($sidebarTitle))', $result);
    }

    public function testNamedFragmentExtractsFirstFragment(): void
    {
        // Arrange
        $compiler = new TemplateCompiler();
        $source = file_get_contents(self::$fixtureDir . '/page-with-fragments.html');
        ...
        // Act
        $result = $compiler->compile($source, self::$fixtureDir, fragmentName: 'product-list');

        // Assert — only the product list fragment
        $this->assertStringContainsString('<?php foreach ($products as $product): ?>', $result);
        $this->assertStringNotContainsString('aside', $result);
    }

    public function testNamedFragmentReturnsEmptyForUnknownName(): void
    {
        // Arrange
        $compiler = new TemplateCompiler();
        $source = file_get_contents(self::$fixtureDir . '/page-with-fragments.html');
        ...
        // Act
        $result = $compiler->compile($source, self::$fixtureDir, fragmentName: 'nonexistent');

        // Assert — empty string, not an error (fall-through handled by renderer)
        $this->assertSame('', $result);
    }

    public function testNamedFragmentWithConditionalContent(): void
    {
        // Arrange
        $compiler = new TemplateCompiler();
        $source = "{{ fragment 'alert' }}{{ if \$show }}<div>{{ \$message }}</div>{{ endif }}{{ endfragment }}";

        // Act
        $result = $compiler->compile($source, self::$fixtureDir, fragmentName: 'alert');

        // Assert — conditional compiled inside the fragment
        $this->assertStringContainsString('<?php if ($show): ?>', $result);
        $this->assertStringContainsString('$__escape((string)($message))', $result);
        $this->assertStringContainsString('<?php endif; ?>', $result);
    }

    public function testNamedFragmentWithInclude(): void
    {
        // Arrange
        $compiler = new TemplateCompiler();
        $source = "{{ fragment 'nav-block' }}{{ include 'partials/nav' }}{{ endfragment }}";

        // Act
        $result = $compiler->compile($source, self::$fixtureDir, fragmentName: 'nav-block');

        // Assert — include was resolved inside the fragment
        $this->assertStringContainsString('<nav>', $result);
    }

    public function testNamedFragmentWithoutExtendsSearchesEntireSource(): void
    {
        // Arrange — template with no layout, fragments in raw source
        $compiler = new TemplateCompiler();
        $source = "<h1>Title</h1>\n{{ fragment 'detail' }}<p>{{ \$info }}</p>{{ endfragment }}";

        // Act
        $result = $compiler->compile($source, self::$fixtureDir, fragmentName: 'detail');

        // Assert
        $this->assertStringContainsString('$__escape((string)($info))', $result);
        $this->assertStringNotContainsString('<h1>Title</h1>', $result);
    }

    public function testMultipleFragmentsPerSection(): void
    {
        // Arrange
        $compiler = new TemplateCompiler();
        $source = file_get_contents(self::$fixtureDir . '/page-with-fragments.html');
        ...
        // Act — compile each fragment independently
        $productList = $compiler->compile($source, self::$fixtureDir, fragmentName: 'product-list');
        $sidebar = $compiler->compile($source, self::$fixtureDir, fragmentName: 'sidebar');

        // Assert — each fragment has its own content, no overlap
        $this->assertStringContainsString('foreach', $productList);
        $this->assertStringNotContainsString('aside', $productList);

        $this->assertStringContainsString('aside', $sidebar);
        $this->assertStringNotContainsString('foreach', $sidebar);
    }

    public function testFragmentDelimitersStrippedInFullRender(): void
    {
        // Arrange — full render (not fragment mode) should treat fragment
        // markers as transparent and render their content inline.
        $compiler = new TemplateCompiler();
        $source = "<div>{{ fragment 'box' }}<p>{{ \$name }}</p>{{ endfragment }}</div>";

        // Act
        $result = $compiler->compile($source, self::$fixtureDir);

        // Assert — no fragment markers in output, content rendered inline
        $this->assertStringNotContainsString('fragment', $result);
        $this->assertStringNotContainsString('endfragment', $result);
        $this->assertStringContainsString('<div>', $result);
        $this->assertStringContainsString('$__escape((string)($name))', $result);
    }

    public function testFragmentDelimitersStrippedInSectionFragmentMode(): void
    {
        // Arrange — the boolean fragment mode (content section only, no layout)
        // should also strip fragment delimiters.
        $compiler = new TemplateCompiler();
        $source = "{{ extends 'layout' }}\n{{ section 'title' }}T{{ endsection }}\n"
            . "{{ section 'content' }}{{ fragment 'x' }}<p>Hi</p>{{ endfragment }}{{ endsection }}";

        // Act
        $result = $compiler->compile($source, self::$fixtureDir, fragment: true);

        // Assert — content section only, fragment markers gone
        $this->assertStringNotContainsString('fragment', $result);
        $this->assertSame('<p>Hi</p>', $result);
    }

    public function testUnclosedFragmentThrowsException(): void
    {
        // Arrange
        $compiler = new TemplateCompiler();
        $source = "{{ fragment 'open' }}<p>content</p>";

        // Act & Assert
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage("Unclosed fragment block: '{{ fragment 'open' }}'");
        $compiler->compile($source, self::$fixtureDir, fragmentName: 'open');
    }

    public function testOrphanEndfragmentThrowsException(): void
    {
        // Arrange
        $compiler = new TemplateCompiler();
        $source = "<p>content</p>{{ endfragment }}";

        // Act & Assert
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage("'{{ endfragment }}' found without a matching '{{ fragment }}'");
        $compiler->compile($source, self::$fixtureDir, fragmentName: 'anything');
    }

    public function testNestedFragmentsThrowException(): void
    {
        // Arrange
        $compiler = new TemplateCompiler();
        $source = "{{ fragment 'outer' }}{{ fragment 'inner' }}<p>X</p>{{ endfragment }}{{ endfragment }}";

        // Act & Assert
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage("opened inside");

        $compiler->compile($source, self::$fixtureDir, fragmentName: 'outer');
    }

    public function testFragmentValidationRunsInFullRenderToo(): void
    {
        // Arrange — unclosed fragment should fail even in full-render mode
        $compiler = new TemplateCompiler();
        $source = "{{ fragment 'broken' }}<p>hi</p>";

        // Act & Assert
        $this->expectException(\RuntimeException::class);
        $this->expectExceptionMessage("Unclosed fragment block");
        $compiler->compile($source, self::$fixtureDir);
    }
}
