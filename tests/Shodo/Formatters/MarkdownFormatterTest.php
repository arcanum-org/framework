...
final class MarkdownFormatterTest extends TestCase
{
    ...
    {
        $base = sys_get_temp_dir() . '/arcanum_md_formatter_test_' . uniqid();
        $this->rootDir = $base;
    ...
    private function createFormatter(?HelperResolver $helpers = null): MarkdownFormatter
    {
        $resolver = new TemplateResolver($this->rootDir, 'App', extension: 'md');
        $compiler = new TemplateCompiler();
        $cache = new TemplateCache($this->cacheDir);
        $fallback = new MarkdownFallbackFormatter();

        return new MarkdownFormatter($resolver, $compiler, $cache, $fallback, helpers: $helpers);
    }
    ...
    public function testFormatsTemplateWithArrayData(): void
    {
        // Arrange
        file_put_contents(
            $this->rootDir . '/app/Pages/Index.md',
            '# Welcome to {{ $name }}!',
        );
        $formatter = $this->createFormatter();
    ...
    public function testTemplateDoesNotEscapeHtmlCharacters(): void
    {
        ...
        // Assert — no HTML escaping in Markdown
    ...
    public function testHelperCallResolvesFromResolver(): void
    {
        ...
        // Assert
        $this->assertSame('[Health](/api/health)', $result);
    }
    ...
    public function testHelperOutputUsesIdentityEscape(): void
    {
        ...
        // Assert — no HTML encoding in Markdown
