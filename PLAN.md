...
- [x] **Migrate `tests/Integration/CqrsLifecycleTest.php` to `TestKernel`.** Full-lifecycle tests now dispatch through `TestKernel->http()->get/put(...)` and assert on the returned `ResponseInterface`, instead of hand-rolling `stubRequest()` + per-test `MiddlewareBus` + `Hydrator` + `Router` + `FormatRegistry` construction. A new `tests/Fixture/Integration/RoutingHandler.php` fixture wires the Route → Hydrate → Dispatch → Render pipeline once and is installed via `setCoreHandler()` from a single `buildKernel()` helper — every test now reads as "build kernel, dispatch, assert on response." Five sub-piece tests (`testJsonBodyHydratesCommandDto`, `…WithDefaults`, `testCommandDispatchReturnsEmptyDto`, `testQueryParamsHydrateQueryDto`, `testQueryDispatchReturnsWrappedResult`) deleted as redundant — every code path they exercised is fully covered by the seven full-lifecycle tests that remain. Two `RouteDispatcher` tests stayed as direct unit tests (they assert per-route middleware composition around the bus, not the HTTP boundary) and were minimally rewritten to construct only what they need. Suite green at 2539 tests; net -5 from the redundancy collapse.
- [x] **Migrate `tests/Integration/HelperResolutionTest.php` to `TestKernel`.** Smaller win than `CqrsLifecycleTest` because this test isn't dispatching through the HTTP boundary — it bootstraps `Routing` + `Helpers` manually and asserts on `HelperResolver` behavior. Migration was a minimal swap of `new Container()` for `$kernel = new TestKernel(rootDirectory: $this->rootDir); $container = $kernel->container();`. Proves TestKernel slots in cleanly even for bootstrapper-driven setup, and gives the test the same shared bindings (FrozenClock / ArrayDriver / ActiveIdentity) any future app handler test will inherit — even though this particular test doesn't use them. All 6 tests still pass; suite green at 2539.
- [x] **Evaluate `tests/Hyper/Event/LifecycleEventTest.php` for migration.** Migrated. All seven tests now use `TestKernel` with `CapturingTestHandler` as the core handler — `CapturingTestHandler` already records the dispatched request, which is exactly what `CapturingKernel` provided here. The `RequestFailed` test relies on the unrouted-404 path with `RenderingExceptionRenderer` registered. The two `ResponseSent` tests required a small `HttpTestSurface::terminate()` addition (delegates straight to the wrapped kernel) — worth doing on its own merits since apps that hook into post-response listeners will need it. `CapturingKernel` stays in place because `tests/Ignition/HyperKernelTest.php` still uses it; only `LifecycleEventTest` lost the dependency. Suite green at 2540.
- [x] **Migrate starter app `tests/Helpers/WiredUpHelperTest.php` to `TestKernel`.** Required a small framework prep first: TestKernel now implements `Kernel` (the interface only requires four directory/env accessors plus no-op `bootstrap`/`terminate`, all easy to satisfy because TestKernel owns its own pre-built container) and auto-binds itself as `Kernel::class`. With that in place the migration is a one-liner: `new FixtureKernel(...)` becomes `new TestKernel(rootDirectory: ...)`. Starter app commit: `a757226`. Framework commit: `41219e5`.
- [x] **Migrate starter app `tests/Helpers/EnvCheckHelperTest.php` to `TestKernel`.** Same one-line swap as `WiredUpHelperTest`.
- [x] **Delete starter app `tests/Helpers/FixtureKernel.php`.** Removed in the same starter app commit (`a757226`). Verified no remaining references via grep. Future helper tests must reach for `TestKernel` instead of inventing a new ad-hoc fake.
