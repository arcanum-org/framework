...
- [ ] **JsonFormatter JSON_UNESCAPED_SLASHES** — `</script>` passes through JSON unescaped. If JSON is embedded in HTML, this is an XSS vector. Fix: add `JSON_HEX_TAG` flag to `json_encode()`.
