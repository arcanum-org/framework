...
- [x] `Connection` — interface defining the contract for database connections. Methods: `run(string $sql, array $params): Result`, `beginTransaction(): void`, `commit(): void`, `rollBack(): void`. All consumers (Model, Database, ConnectionManager) depend on this interface, not a concrete implementation.
- [x] `PdoConnection` — built-in `Connection` implementation wrapping PDO. Lazy-connects on first use. Sets `ERRMODE_EXCEPTION`, `FETCH_ASSOC` defaults. Ships with the framework for zero-dependency setups. App developers can provide their own `Connection` implementation (e.g., wrapping Doctrine DBAL) for advanced driver support.
- [x] `ConnectionFactory` — creates `Connection` instances from config arrays. Supports drivers: `mysql` (charset, collation), `pgsql`, `sqlite` (file path or `:memory:`). Builds DSN string from config keys (`host`, `port`, `database`, `unix_socket`, etc.). Returns `PdoConnection` by default; apps can replace the factory to return DBAL-backed connections.
