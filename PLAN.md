...
- [x] **CSRF/Auth coordination for token-authenticated requests** — AuthMiddleware sets a PSR-7 request attribute (`auth.token_authenticated`) when a TokenGuard successfully resolves an identity. CsrfMiddleware reads it and skips CSRF validation. CompositeGuard tracks which inner guard resolved via `lastResolvedGuard()`. Guard config supports array syntax: `'guard' => ['session', 'token']`.
...
- [x] **Dev mode unused template variable warning** — TemplateAnalyzer scans template source inside `{{ }}` delimiters for `$variable` references, compares with passed variables, triggers `E_USER_NOTICE` listing unused vars. Scans source (not compiled PHP) to avoid false positives from `$` in raw text. Debug mode only — zero production overhead.
