...
- [ ] **Add `Hourglass\Interval` with `secondsIn` and `ofSeconds`.** Final class, two static methods. `secondsIn(\DateInterval): int` accepts any `DateInterval` and returns total seconds via the existing epoch-anchor trick. `ofSeconds(int): \DateInterval` constructs a fresh `DateInterval` from a positive integer count of seconds. New `IntervalTest` covers both methods, including the documented epoch-anchor behavior for month and year intervals (1 month → 31 days, 1 year → 365 days).
- [ ] **Migrate all four Vault drivers to call `Interval::secondsIn()`.** Replace the inline incantation in `ApcuDriver::resolveTtl`, `RedisDriver::resolveTtl`, `ArrayDriver::resolveExpiry`, and `FileDriver::resolveExpiry`. Drop the now-stale explanatory docblocks at `ApcuDriver`/`RedisDriver` `resolveTtl` since the call site is self-explanatory.
