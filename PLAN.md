...
- [ ] **Build `Factory`.** Composes `Codex\Hydrator` per the design decision above: pre-pass walks constructor params, synthesizes values for any param without an override and without a default (validation-attribute-driven for the easy rules, recursive `Factory::make` for nested DTO types, `null` for nullable types), then hands `array_merge($synthesized, $overrides)` to `Hydrator::hydrate()`. `Factory::make(class-string<T>, array $overrides = []): T` is the public surface. Constructor takes `Hydrator $hydrator = new Hydrator()` for ergonomics. Tests cover happy path, override path, recursive nested-DTO path, default-fallback path, and the `FactoryException` path for `#[Pattern]` against a non-trivial regex and for `#[Callback]` rules.
...
- **`TestKernel` transactional database wrapping** — Laravel's `RefreshDatabase`, Symfony's DAMA DoctrineTestBundle, and CakePHP's fixture system all solve the same problem: keep test database state isolated by wrapping each test in a transaction and rolling back at teardown. When Forge grows real-database integration tests, `TestKernel` should ship a transactional opt-in (`->withTransactionalDatabase()` or similar) that does the same. Out of scope for the current testing-utilities arc — Arcanum doesn't yet have enough Forge usage to justify the design — but worth flagging now so the precedent isn't forgotten when the need arises. The pattern is well-trodden across all major PHP frameworks.
