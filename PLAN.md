...
- [ ] **Welcome-page upscales: diagnostic row re-check.** Add per-row re-check buttons. Each row gets a unique id (e.g., `<div id="diag-database">`). Buttons target their own row's id. `GetIndexHandler` unchanged — auto-extraction serves the right element.
- [ ] **Welcome-page upscales: CQRS demo lazy-load.** The existing CSS-only tabs become htmx-lazy-loaded. Each tab's content lives inside a `<div id="cqrs-tab-x">` block. Tab buttons get `hx-get="/" hx-target="#cqrs-tab-x" hx-trigger="click once"` so each tab loads on first click and stays cached. Auto-extraction serves each tab independently.

#### Lazy template data

- [ ] **Add lazy closure support to the template executor.** When a handler returns a closure as a template variable value, the executor defers invocation until the variable is actually accessed during rendering. If auto-fragment extraction means the variable's element is never rendered, the closure never runs and the underlying query never executes. This eliminates the performance footgun where a handler loads 1000 rows for `$items` but the htmx request only targets `tiny-footer` which uses `$simpleVariable`. Implementation: a `Lazy` wrapper (or direct closure detection in `extractVariables`) that produces proxy values; the compiled template's `extract()`'d scope triggers invocation on first read. Handler code: `'items' => fn() => $this->db->model->items(...)->toSeries()->all()`. No htmx awareness, no element-id coupling, fully transport-agnostic — the optimization is automatic. The existing debug-mode undefined-variable detection should still work (a lazy variable that exists but was never invoked is not "undefined"). Design and test carefully — PHP's `extract()` + closure interaction needs a clean proxy pattern. **Two known concerns:** (1) If a lazy closure calls a model method that dispatches Echo events, those events fire during the render pass, not the handler pass — the `HtmxEventTriggerMiddleware` capture window may already be closed, so the event capture design must account for render-time events. (2) Lazy closures move database queries into the rendering pass, blurring the "rendering is pure string output" boundary — a database exception during rendering surfaces through the exception rendering path rather than the handler error path, and Glitch needs to handle that gracefully. Neither is a blocker, but both are load-bearing constraints.
