...
- [x] **Create `Arcanum\Shodo\TemplateEngine`.** Four methods: `render()`, `renderFragment()`, `renderElement()`, `renderSource()`. Owns compilation, caching, execution, and closure resolution (selective for partials, full for complete renders). Skips `__`-prefixed variables during closure resolution.
...
- [x] **Remove manual `setDtoClass()` wiring from starter app kernel.** The framework now threads DTO class context automatically via `RouteDispatcher`. The starter app's `Kernel::handleRequest()` no longer needs to catch exceptions and call `setDtoClass()` on the renderer — remove that ceremony.
...
### Kernel lifecycle events & shared Lifecycle class — complete

Closed the CLI observability gap: RuneKernel now dispatches lifecycle events and taps Stopwatch marks, parallel to HyperKernel's HTTP lifecycle. A shared `Lifecycle` class provides container-aware event dispatching and exception reporting for both kernels, simplifying HyperKernel (511 → 448 lines, 21 → 16 methods).

- [x] **Create CLI lifecycle event classes.** `CommandReceived(Input)`, `CommandHandled(Input, int exitCode)`, `CommandFailed(Input, Throwable)`, `CommandCompleted(Input, int exitCode)` in `src/Rune/Event/`. All extend `Echo\Event`, all read-only with getters. Tests for each.
- [x] **Create `Arcanum\Ignition\Lifecycle`.** `dispatch(object $event): object` checks container for `EventDispatcherInterface`, dispatches, returns event. `report(\Throwable $e): void` checks container for `ExceptionHandler`, reports. No caching of the dispatcher — Htmx decorates it after bootstrap.
- [x] **RuneKernel composes Lifecycle and dispatches CLI events.** Lifecycle created in `bootstrap()`. `CommandReceived` fires after empty-command check, before dispatch. `CommandHandled` fires after successful dispatch. `CommandFailed` fires in catch block. `CommandCompleted` fires in `terminate()`. Stopwatch marks: `command.received`, `command.handled`, `command.completed`. Events fire for both BuiltInRegistry and domain commands.
- [x] **HyperKernel composes Lifecycle, drops 5 private methods.** Replaced `dispatchRequestReceived`, `dispatchRequestHandled`, `dispatchRequestFailed`, `dispatchEvent`, `reportException` with inline `Lifecycle::dispatch()` / `Lifecycle::report()` calls. Stopwatch marks stay inline.
- [x] **Update COMPENDIUM.** CLI lifecycle events in Rune description, Lifecycle in Ignition description, CLI Stopwatch marks in Hourglass description.
- [x] **Update PLAN.** This checklist.
- [x] **`composer check` passed after each commit.**
...
- [x] **Tailwind styling on active tabs not working.** Root cause: labels were nested inside a wrapper `<div>`, breaking Tailwind's `peer-checked/name:` variant which requires direct siblings. Fix: moved labels to be flat siblings of the radio inputs, CSS grid for visual tab layout. Works with Tailwind v4.
...
- [ ] **Replace custom copy-to-clipboard JS with htmx** — the welcome page's "copy" links use inline JavaScript. Investigate whether htmx (or a lightweight htmx extension) can handle clipboard copy without custom JS. If not, at least move the JS out of inline handlers.
...
- **Refactor `HyperKernel` and `RuneKernel` onto a shared `AbstractKernel` base** — The two kernels still share identical constructor, `isBootstrapped` flag, four directory accessors, and bootstrap loop (differing only in `Transport` enum). The Lifecycle extraction (April 2026) already unified event dispatching and exception reporting, reducing the shared surface. What remains for `AbstractKernel`: constructor, directory accessors, `requiredEnvironmentVariables()`, `bootstrap()` (with abstract `transport(): Transport`), and the `Lifecycle` property initialization. Estimated ~80 lines of deduplication. Pure refactor; no API changes. TestKernel exercises both kernels through composition, so verification is in place.
