...
- **Testing utilities** — DTO factories, service fakes, TestKernel. Groups naturally with the PSR-20 `Clock` adoption pass (Sessions, Auth, Throttle, Vault, Forge timestamps each currently call `time()` / `new DateTime()` directly — wiring them to the Hourglass `Clock` interface makes the whole framework deterministic for tests).
