...
- [ ] **`#[AllowedFormats]` attribute for format restriction** — DTO attribute like `#[AllowedFormats('html', 'json')]` checked by the router. Requesting a disallowed format returns 406 Not Acceptable. No attribute = all formats allowed (backwards compatible). Prevents handlers from leaking data via unexpected format switches.
