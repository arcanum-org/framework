...
- [ ] **Build the minimal `TestKernel` core.** Constructor with optional overrides (clock, cache, root directory). Builds the shared container up front, binds `FrozenClock` (defaults to a fixed `2026-01-01` instant), binds `ArrayDriver` as the cache, registers `ActiveIdentity`. Provides `container()`, `clock()`, and an `actingAs(Identity)` chained setter (named to follow the universal convention from Laravel/Symfony/Cake — reads naturally in tests). No HTTP or CLI surfaces yet — just the kernel base. `tests/Testing/TestKernelTest.php` covers construction + accessors.
