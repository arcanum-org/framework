...
- **Correlate with a request ID.** Generate a short random ID at the start of `handle()` and include it as `'request_id'` in every log context for that request. Without this, interleaved DEBUG lines from concurrent requests are useless. Implement as a Monolog processor registered in Bootstrap\Logger so it applies to all channels automatically.
