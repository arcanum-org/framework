...
- [ ] **Add `CompilerDirective` interface to Shodo.** Minimal plugin contract: `directives(): array` (the directive names the plugin claims, e.g. `['fragment', 'endfragment']`) and `process(string $source): string` (pre-pass that strips/transforms the claimed directives). The `TemplateCompiler` holds a list of registered plugins and runs each before the main regex passes. Unknown `{{ lowercase_keyword }}` directives that aren't claimed by any plugin produce a clear error ("unknown directive 'fragment'") instead of hitting the escaped-output fallback.
