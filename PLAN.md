...
- **Align `HelperDiscovery` with `MiddlewareDiscovery`** — Today the two convention-based discovery systems use *different* rules. `MiddlewareDiscovery` walks the whole `app/` tree, so `app/Pages/Middleware.php`, `app/Domain/Shop/Middleware.php`, and a top-level `app/Middleware.php` all work via uniform namespace-prefix scoping. `HelperDiscovery` walks `app/Domain/` only, so a `Helpers.php` outside that subtree is invisible — Pages cannot have their own scoped `Helpers.php` via discovery. To compensate, `Bootstrap\Helpers` reads a special hardcoded path `app/Helpers/Helpers.php` and registers its contents as global helpers. The result is a design wart: two conventions that look identical from the outside but follow incompatible rules. Fix is to teach `HelperDiscovery` to walk `app/` like `MiddlewareDiscovery` does, then drop the special-path read in `Bootstrap\Helpers` (or keep it as a deprecated convenience, then remove it). After the change, `app/Helpers.php` would be the natural top-level home and `app/Pages/Helpers.php` would Just Work. Low-medium priority — current behavior is documented and shipping, but the asymmetry will trip up every new app dev.
